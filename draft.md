...
Click _Authorize_ button to connect your repo to Azure Pipelines.
...
Choose to install Azure Pipelins into all repositories or just into a single repo.
...
After all this initial steps, the fun part can finally begin.
...
For now, our build does nothing usefull. Let's change that.
...
To make sure that k6 is installed properly we can added a new script task that just outputs version of our k6 install. This task is optional and can be removed in real-life scenarios.

In the example in this article I'm using a single-job build so I'm ommiting "jobs" section.
...
We are naming this test `local.js` since it is being run locally straigt from Azure Pipelines agent.
...
If you got this screen, congrats! You now know how to setup a GitHub project CI build to run on Azure Pipelines and do some sweat load testing.
...
In `azure-pipelines.yml` config file add additional step or modify your existing step that was running k6 locally.
...
In order to get the token, log in into your account on [loadimpact.com](https://loadimpact.com) or create it if you don't have one already. New users can use 30-day free trial period that supports testing from multiple load zones.
...
Now we need to add a Azure Pipelines variable that will be available within the build.
...
Once you enter the options for your project switch to _Variables_ tab within the web UI and enter a new secret variable that will used during k6 cloud execution.
...
Don't forget to set variable as secret so that it's not visible as plain text in your pipelines output.
